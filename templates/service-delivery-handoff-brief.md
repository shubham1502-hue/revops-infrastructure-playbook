# Service Delivery Handoff Brief

**Instructions:** This brief is auto-populated from the CRM cloned deal. The Sales rep must fill all mandatory fields before the Closed Won handoff can trigger.

---

## Client Information

| Field | Value |
|---|---|
| Client / Organization Name | |
| Primary Contact | |
| Contact Phone | |
| Contact Email | |
| Region | |

---

## Deal Summary

| Field | Value |
|---|---|
| Deal Name | |
| Close Date | |
| Payment Terms | |
| Delivery Timeline Promised | |

---

## Delivery Requirements

_What exactly was committed to the client? Include product specs, quantities, timelines, and any custom requirements agreed during the sales process._

[Free text - mandatory]

---

## Inventory Requirement

_What products or materials are needed for delivery? Include SKUs, quantities, variants._

[Free text - mandatory]

---

## Procurement Notes

_Any supplier-specific notes, lead times, or constraints the procurement team should know._

[Free text - optional]

---

## Special Commitments

_Anything agreed during sales that is non-standard or outside the normal delivery process._

[Free text - optional]

---

## What NOT to include in this brief

This handoff brief is visible to Delivery, Inventory, and Procurement teams. Do not include:
- Final close price or pricing negotiation history
- Proposal amount or discount details
- Internal rep notes or sales strategy commentary

---

**Completed by (Sales Rep):** _______________
**Handoff Date:** _______________
**Acknowledged by (Delivery):** _______________
